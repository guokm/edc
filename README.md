...
- 运营方入口：运营主数据、治理接口巡检、按次计费校验、节点与健康监控
- 默认试点账号：`operator_admin` / `provider_admin` / `consumer_admin`，初始密码均为 `ChangeMe@123`
- 未登录时会显示角色账号卡片；登录后自动收敛到当前账号所属角色门户
- 每个角色都提供 `角色演示文档` 页面，支持现场讲解
- 一键全流程演示提供确认弹框，先展示步骤说明再执行
- 节点与健康页提供运行态摘要、异常项聚焦、双 Data Plane 承载统计
...
- 控制面/数据面/身份/发行/联邦/运营：仅暴露在 Docker 网络内，不映射宿主机端口
