...
- 页面：`治理模块接口`、`节点与健康`
- 操作顺序：
  1. 使用 `operator_admin / ChangeMe@123` 登录运营账号（试点部署后必须修改初始密码）
  2. 查看组织、参与方、账号列表，确认 `operator`、`participant-a`、`participant-b` 已绑定
  3. 一键巡检治理接口
  4. 执行额度校验（按次计费）
  5. 查看双数据平面与健康摘要
...
1. `运营方入口` 先使用 `operator_admin / ChangeMe@123` 登录，拿到运营令牌 `X-Operator-Token`。  
   预期：当前用户为 `PLATFORM_ADMIN`，绑定参与方 `operator`。
2. 在运营方页面查看 `组织列表 / 参与方列表 / 账号列表`。  
   预期：组织有“数据空间运营平台、华东车联、保险风控中心”；参与方有 `operator`、`participant-a`、`participant-b`。
3. `运营方入口 -> 治理模块接口`，点击 `一键巡检`。  
   预期：Identity/Issuer/Federated/Operator 检查项返回 200。
4. 在同页点击 `创建会员`，再点 `会员列表`。  
   预期：出现 `mem-xxx`，`participantId=participant-b`，状态 `ACTIVE`。
5. 在同页点击 `执行签发`（Issuer）。  
   预期：返回 `issuanceId`，状态 `ISSUED`。
6. 在同页点击 `写入凭证`（Identity）。  
   预期：返回 `credentialId`，凭证写入成功。
7. 在同页点击 `创建DCP展示` 和 `校验DCP展示`。  
   预期：返回 `presentationId`，并看到 `verified=true`。
8. 切到 `供应方入口 -> 供应方工作台`，刷新/创建策略后发布资产。  
   预期：发布轨迹出现 `创建资产 -> 写入 Offer -> 刷新目录`。
9. 切到 `消费方入口 -> 消费方工作台`，刷新目录并选择资产与 Offer，执行 `签发并签约`。  
   预期：协商状态 `FINALIZED`，生成 `agreementId`。
10. 在消费方页面选择协议，点击 `发起传输`，再 `拉取数据`。  
   预期：出现 `transferProcessId`，链路状态推进并拿到 payload。
11. 回到 `运营方入口`，点击 `额度校验`，再到 `节点与健康` 刷新。  
