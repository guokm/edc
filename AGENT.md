...
- 顶部已升级为**登录态 + 角色入口 + 角色内导航**：
  - 未登录：展示 3 张演示账号卡片（供应方 / 消费方 / 运营方），点击可自动填充账号密码
  - 已登录：顶部展示当前用户、participantId、角色编码、过期时间与退出按钮
  - 角色入口按登录账号收敛，非当前角色卡片置灰，不允许误切
- 顶部改为**角色入口 + 角色内导航**：
  - `供应方入口`：
  ...
  - `运营方入口`：
    - `运营主数据`：组织 / 参与方 / 账号绑定关系，用于解释“谁登录、谁代表谁操作”
...
- 后端服务端口只暴露在 Docker 网络内，不映射宿主机端口（避免与服务器已有 818x 容器冲突）
...
- 当前最新小版本说明：`docs/V1.1.1/version-notes.md`（运营登录与角色门户版）。
